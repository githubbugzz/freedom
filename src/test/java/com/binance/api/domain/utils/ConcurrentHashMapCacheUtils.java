...
import com.binance.api.domain.constant.Signal;
...
public class ConcurrentHashMapCacheUtils {
     ...
     * 是否正在买卖
     */
    static Signal signal = Signal.NOTRADING;
...
//    private static ExecutorService executor = Executors.newSingleThreadExecutor();
    ...
    private static void checkTriggerTrade() {
        List<AggTradeEvent> list = new ArrayList();
        for(Map.Entry<String,CacheObj> entry :CACHE_OBJECT_MAP.entrySet()){
            list.add((AggTradeEvent)entry.getValue().getCacheValue());
        }
        double sum=0;
        for(AggTradeEvent tradeEvent :list){
            if(tradeEvent.isBuyerMaker()){
                sum-=Double.valueOf(tradeEvent.getQuantity());
            }else{
                sum+=Double.valueOf(tradeEvent.getQuantity());
            }
        }
        LOGGER.info("当前10s交易数量:"+sum);
//        LOGGER.debug("当前15s交易数量:"+sum);
        if(sum>17){
            //可以启动线程进行买入操作。
            LOGGER.debug("当前10s交易数量:"+sum);
            LOGGER.info("可以进行买入操作。。");
            switch (signal){
                case  LONG:
                    break;
                case SHORT:
                    LOGGER.debug("停止做空交易，开始做多");//貌似这样不是太合理，是不是sum>0的时候就停止？
                    signal=Signal.LONG;
                case NOTRADING:
                    LOGGER.debug("开始做多");
                    signal=Signal.LONG;
    ...
    static void deleteTimeOut() {
        ...
        LOGGER.info("cache current count is :" + CURRENT_SIZE);
    }
